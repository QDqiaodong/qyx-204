...
8. **公共洗漱台送检台账**：开单须选中一台、写清损坏部位和经办值班员。送检单只有"待接单 / 已修复"两态；同一台只要还有未结送检就不能再开第二张（重复开单会带出上一张的单号与送检起始时间），并发开单由数据库唯一约束保证只留下一张。未结期间匹配一览不再把该台计入任何单元的配套洗漱台与容量，但原绑定关系与历史痕迹不改、不删，只另补送检/修复痕迹；登记修复后该台自动重新计入配套，也才能再次送检
...
- `POST /api/repair-orders` - 开送检单（洗漱台、损坏部位、经办值班员）；同台已有未结单或并发抢开时返回 400
- `POST /api/repair-orders/{id}/complete` - 登记修复（待接单 -> 已修复，可填修复备注）
- `GET /api/repair-orders` - 送检台账（可按洗漱台 / 状态 PENDING、REPAIRED / 楼栋筛选）
- `GET /api/repair-orders/current?washbasinId=` - 查询某台当前未结送检单（带出送检起始时间）
