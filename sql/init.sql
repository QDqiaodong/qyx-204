...
-- 洗漱台送检单：同一台至多一张未结单（open_flag=1），已结单 open_flag 为 NULL，可并存多张
...
CREATE TABLE IF NOT EXISTS matching_check_record (
    ...
    check_type VARCHAR(50) NOT NULL COMMENT '校验类型：BIND/UNBIND/UPDATE/REPAIR_OPEN/REPAIR_DONE',
    unit_resident_count INT NOT NULL COMMENT '单元居住人数',
    total_capacity INT NOT NULL COMMENT '总容纳容量',
    check_result VARCHAR(20) NOT NULL COMMENT '校验结果：PASS/WARN/FAIL/INFO',
